...
an angle step (rad/sample) → Break it (aliasing) → **Up the octaves** (log slider
to ±20 kHz with auto slow-motion; real-world anchors from heartbeat to A440 to a
CRT whine) → Playground.
...
Frequency slider (linear ±8 Hz in the lessons, logarithmic to ±20 kHz in the last
two stages), "sample it" toggle, strobe-rate slider, "strobe view" (hide the
...
|---|---|
| `state()` | `{ stage, taskDone, wide, F, Fs, sampling, strobe, paused, t, phase, radPerSample, wrapped, apparentF, aliased, samples, history, visible[] }` |
...
`aliased`, `apparentF === −1`, `wrapped === −π/4`; pause freezes `t`.
